...
<tr>
  ...
  const renderRequestsTable = (rows) => {
    ...
    if (!rows.length) {
      requestsBody.innerHTML = '<tr><td colspan="9" class="text-center">No pending requests</td></tr>';
      return;
    }
...
<tr>
  ...
  <div class="details-card">
    ...
    <div class="details-list">
      ...
      <div><span>Phone:</span> ${item.phone || '-'}</div>
      <div><span>Location:</span> ${item.location || '-'}</div>
      <div><span>Account Type:</span> ${item.account_type || '-'}</div>
      <div><span>Profile Completion:</span> ${item.profile_completion || 0}%</div>
    </div>
  </div>`;
  ...
  <div class="details-card">
    <h6>Verification</h6>
    <div class="details-list">
      <div><span>KYC Status:</span> ${item.kyc_status || 'Pending'}</div>
      <div><span>Account Status:</span> ${item.account_status || 'Active'}</div>
      <div><span>Warning Count:</span> ${item.warning_count || 0}</div>
    </div>
  </div>`
      : `
  <div class="details-card">
    ...
    <div class="details-list">
      <div><span>Status:</span> ${item.account_status || 'Active'}</div>
      <div><span>Warning Count:</span> ${item.warning_count || 0}</div>
      ${hasResume ? `<div><span>Resume:</span> ${resumeStatus}</div>` : ''}
    </div>
  </div>`;
  ...
  <div class="details-card">
    <h6>Subscription</h6>
    <div class="details-list">
      <div><span>Plan:</span> ${item.plan_name || 'N/A'}</div>
      <div><span>Plan Type:</span> ${item.plan_type || 'N/A'}</div>
      <div><span>Payment:</span> ${item.payment_status || 'N/A'}</div>
    </div>
  </div>`
      : '';
...
<div class="table-wrap" style="margin-bottom:16px;">
  <table class="table">
    ...
    return `
<div class="details-grid">
  ${[profileCard, verificationCard, subscriptionCard].filter(Boolean).join('')}
</div>
